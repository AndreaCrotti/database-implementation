...
** 3. Insert data entry 60*
   ...
   - wl A
# Not very sure it works like that, in the simple tree locking
# we can't unlock until we're sure that the node is safe
# and we only know it in the leaf node
   - wl B => B is safe,
   - wul A
   - wl C
   - wl E  => E is safe, so:
...
# given that we always lock from root to leaf and unlock
# from leaf to root
