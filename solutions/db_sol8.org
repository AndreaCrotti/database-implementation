...
# the join is done in the from (can be explicited also
#+begin_src sql
    select distinct person_name, city from (lives, works)
    where (works.salary < 10000 and works.company_name='MyComp')
#+end_src
...
# FIXME: Also consider case where they don't work at all
#+begin_src sql
    select lives.person_name from (lives, works)
    where (works.person_name != 'MyComp'
#+end_src
...
#+begin_src sql
    select m.manager_name from (boss m, lives p, works w)
    where (w.city <> 'Munich' and w.city <> 'Hamburg')
#+end_src
