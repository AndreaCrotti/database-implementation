...
*** (a) with a clustered b-tree of height 3 (matching records are located in one page)
    ...
    I/O $cost = h + 1 = 4$, we only need to go through the tree in vertical and the read the data page.
...
*** (c) Why is sorting-based projection the standard solution in practice (or why is it superior to hash-based projection)?
    ...
    - hashing can even fail if the bucket is too large
    - hashing is less general than sorting and its performances are too dependent on the data distribution
...
** 1. What is the cost of joining Order and Customer using a page-oriented simple nested loops join?
   ...
   $M + M * N = 1000 + 1000 * 200 \approx on 2 * (10^5)$ I/Os
...
** 4. What is the cost of joining Order and Customer using a hash join?
   We first partition both relations, which means we read and write both relations, so we have cost of 2*(10000 + 2000) I/0s. Then we scan matching partitions, read both relations, so we have cost of  10000 + 2000 I/Os. In total, we have cost 36000 I/Os.
